...
//TO DO: Define delimiters 
static const char tokens[] = " ";


//TO DO2: Define data stuctures used for
//implementing priority_queue
typedef struct _priority_queue {
	char *String;  //dynamic allocation for string
	int priority;
...
//TO DO4: Allocate space for string, int
...
void queue_alloc(priority_queue **bufer, int string_size)
{
	*bufer = (priority_queue *)malloc(sizeof(priority_queue));
	...
	(*bufer)->String = malloc(BUFFER_SIZE * sizeof(char));
...
{
	char *command2 = malloc(BUFFER_SIZE * sizeof(char));
...
//Shit all elemnts with one position to left
...
char *top(void)
{
	...
	char *rBuff = malloc(2 * sizeof(char) * BUFFER_SIZE);
...
/*Based on code returned there are 3 cases:*/
	switch (op_code) {
	case INSERT_OP_CODE:	  //insert
		prepare_insert(command);
		break;
	case TOP_OP_CODE:  //top
		tmp = top();
		printf("%s\n", tmp);
...
void read_command(FILE *fin)
{
	size_t Size = 0;
	char separators[] = "\n";
	char *outBuffer = malloc(sizeof(char) * BUFFER_READ);

	Safe_Exit(outBuffer, ERROR_MALLOC);
	char *command = malloc(2 * BUFFER_SIZE * sizeof(char));
